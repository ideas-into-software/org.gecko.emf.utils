...
public class SerializeTypeTest {
	...
	@Test
	public void testSerializeTypeTrue(@InjectService(timeout = 2000) ServiceAware<ExampleModelPackage> examplePackageAware,
			@InjectService(timeout = 2000) ServiceAware<ResourceSet> rsAware) {
		...
		saveOptions.put(EMFJs.OPTION_SERIALIZE_TYPE, true);
		
		String pathToJsonOutputFile = System.getProperty("base.path") + "/data/exampleSerializeTypeTrue.json";
		save(resourceSet, saveOptions, pathToJsonOutputFile, loadedObj);
		
		check(pathToJsonOutputFile, str -> str.contains("\"eClass\":"));
	}
	
	@Test
	public void testSerializeTypeTrueTypeField(@InjectService(timeout = 2000) ServiceAware<ExampleModelPackage> examplePackageAware,
			@InjectService(timeout = 2000) ServiceAware<ResourceSet> rsAware) {
		...
		Map<Object, Object> saveOptions = new HashMap<Object, Object>();
		saveOptions.put(EMFJs.OPTION_SERIALIZE_TYPE, true);
		saveOptions.put(EMFJs.OPTION_TYPE_FIELD, "new_type_field");
		
		String pathToJsonOutputFile = System.getProperty("base.path") + "/data/exampleSerializeTypeTrueTypeField.json";
		save(resourceSet, saveOptions, pathToJsonOutputFile, loadedObj);
		
		check(pathToJsonOutputFile, str -> str.contains("\"new_type_field\":"));
	}
	
	@Test
	public void testSerializeTypeFalse(@InjectService(timeout = 2000) ServiceAware<ExampleModelPackage> examplePackageAware,
			@InjectService(timeout = 2000) ServiceAware<ResourceSet> rsAware) {
		...
		String pathToJsonOutputFile = System.getProperty("base.path") + "/data/exampleSerializeTypeFalse.json";
		save(resourceSet, saveOptions, pathToJsonOutputFile, loadedObj);
