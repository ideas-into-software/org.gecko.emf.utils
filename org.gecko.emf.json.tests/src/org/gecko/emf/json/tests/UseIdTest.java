...
public class UseIdTest {
	...
	@Test
	public void testUseIdTrue(@InjectService(timeout = 2000) ServiceAware<ExampleModelPackage> examplePackageAware,
			@InjectService(timeout = 2000) ServiceAware<ResourceSet> rsAware) {
		...
		String pathToJsonOutputFile = System.getProperty("base.path") + "/data/exampleUseIdTrue.json";
		save(resourceSet, saveOptions, pathToJsonOutputFile, loadedObj);
		
		check(pathToJsonOutputFile, str -> str.contains("\"_id\":"));
	}
	
	@Test
	public void testUseIdFalse(@InjectService(timeout = 2000) ServiceAware<ExampleModelPackage> examplePackageAware,
			@InjectService(timeout = 2000) ServiceAware<ResourceSet> rsAware) {
		...
		Map<Object, Object> saveOptions = new HashMap<Object, Object>();
		saveOptions.put(EMFJs.OPTION_USE_ID, false);
		
		String pathToJsonOutputFile = System.getProperty("base.path") + "/data/exampleUseIdFalse.json";
		save(resourceSet, saveOptions, pathToJsonOutputFile, loadedObj);
